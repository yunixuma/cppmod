...
/*   Updated: 2023/10/27 07:42:46 by Yoshihiro K      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float	BitcoinExchange::getPrice(int month, int day) const {
	...
	while (month > 0 && price == INVALID_AMOUNT) {
		std::map<int, MonthlyData>::const_iterator	it = this->monthly_data_.find(month);
		if (it != this->monthly_data_.end())
			price = it->second.getPrice(day);
